...
#Lendo os shapefiles de treino:
treino_event_0 <- read_sf('MiddleData/not_occurring_06_21_shp_unique.shp')
treino_event_1 <- read_sf('MiddleData/occuring_06_21_shp_unique.shp')

#Lendo o shapefile de teste:
teste_event_0 <- read_sf('MiddleData/not_occurring_data_22_shp_unique.shp')
...
treino_event_0_SA_Albers <- st_transform(x = treino_event_0,
                                        crs =projecao )

treino_event_1_SA_Albers <- st_transform(x = treino_event_1,
                                        crs =projecao )

teste_event_0_SA_Albers <- st_transform(x = teste_event_0,
                                        crs =projecao )
